...
import routeLinesData from '~/utils/routeLines';
...
export function useRoutesData() {
    const { data: schedulesData, pending, error } = useFetch('/api/schedules', {
        server: false, // We can disable SSR fetch if we want it strictly client-side, but let's keep it default which is isomorphic
        default: () => []
    });

    const STOPS = computed<Stop[]>(() => {
        const stopsMap = new Map<string, Stop>();
        const list = (schedulesData.value || []) as any[];
        list.forEach(s => {
            let lat = 0;
            let lng = 0;
    ...
    const ROUTES = computed<Route[]>(() => {
        return (routeLinesData as any[]).map((r: any) => ({
            id: r.id,
            name: r.name,
    ...
    const getRoutesForStop = (stopName: string): string[] => {
        const stopList = (schedulesData.value || []) as any[];
                    ...
                    route.schedules.forEach((schedule: any) => {
                        ...
                        if (days[todayKey]) {
                            ...
                            arrivals.push({
                                ...
                                color: ((ROUTE_COLORS as any)[routeId] || ROUTE_COLORS['default']) as string,
    ...
    return { schedulesData, pending, error, STOPS, ROUTES, getRoutesForStop, getArrivalsForStop, ROUTE_COLORS };
}
